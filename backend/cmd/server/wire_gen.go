...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	openClawBillingRepository := repository.NewOpenClawBillingRepository(client, db)
	openClawBillingService := service.NewOpenClawBillingService(openClawBillingRepository, configConfig)
	openClawTaskSessionRepository := repository.NewOpenClawTaskSessionRepository(client, db)
	openClawTaskSessionService := service.NewOpenClawTaskSessionService(openClawTaskSessionRepository, openClawBillingService, apiKeyService, subscriptionService, groupRepository, settingRepository, configConfig)
	openClawBillingHandler := handler.ProvideOpenClawBillingHandler(openClawBillingService, openClawTaskSessionService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, openClawBillingHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	optionalJWTAuthMiddleware := middleware.NewOptionalJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService, auditLogService)
	apiKeyAuthMiddleware := middleware.NewOpenClawAPIKeyAuthMiddleware(apiKeyService, subscriptionService, configConfig, openClawTaskSessionService)
	...
	openClawBillingLeaseExpiryService := service.ProvideOpenClawBillingLeaseExpiryService(openClawBillingService, openClawTaskSessionService)
